...
def main():
    ...
    for exp in range(args.num_exp):
        ...
        for it in range(args.Iteration+1):
            ...
            # net.train()
            mae_model = Get_MAE(image_size = args.image_size, patch_size = args.patch_size, dim = args.embed_dim, masking_ratio = args.masking_ratio)
            args.mae_model.load_state_dict(torch.load('mae_covidx.pt', map_location=torch.device(args.device)))
            
            net = MAE_encoder(mae_model.encoder, num_classes=args.num_classes)
